...
//Routing Halaman Login Counselors
Router.route('/login-Counselors',
    function () {
        this.render('LoginCounselors', {
            to: "custom"
        });
    },
    { name : 'LoginCounselors'});
...
// Routing Halaman Home Dashboard Counselors
Router.route('/counselors-home',
    function () {
        if(Meteor.user() == undefined){
            Router.go('RootIndex')
        }else {
            this.render('HeaderBK', {
                to: "header"
            });
        }

        this.render('SidebarBK', {
            to: "sidebar",
            data: function(){
                if (this.ready()) {
                    var Currtipeuser = Meteor.user().profile.tipeuser_id;
    ...
    verifyEmail: function() {
        ...
        Accounts.verifyEmail(verificationToken,  function(error) {
            if (error) {
                ...
                sweetAlert("SUCCESS", "AKUN Anda Sukses tervalidasi, Silahkan Login Kembali...")
                Meteor.logout()
